...
@[target, simp]
theorem ack_zero (n : ℕ) : ack 0 n = n + 1 := by sorry

@[target, simp]
theorem ack_succ_zero (m : ℕ) : ack (m + 1) 0 = ack m 1 := by sorry
...
@[target, simp]
theorem ack_two (n : ℕ) : ack 2 n = 2 * n + 3 := by sorry
...
@[target]
theorem one_lt_ack_succ_right : ∀ m n, 1 < ack m (n + 1) := by sorry

@[target]
theorem ack_strictMono_right : ∀ m, StrictMono (ack m) := by sorry

@[target]
theorem ack_mono_right (m : ℕ) : Monotone (ack m) := by sorry

@[target]
theorem ack_injective_right (m : ℕ) : Function.Injective (ack m) := by sorry

@[target, simp]
theorem ack_lt_iff_right {m n₁ n₂ : ℕ} : ack m n₁ < ack m n₂ ↔ n₁ < n₂ := by sorry

@[target, simp]
theorem ack_le_iff_right {m n₁ n₂ : ℕ} : ack m n₁ ≤ ack m n₂ ↔ n₁ ≤ n₂ := by sorry
...
@[target]
theorem add_lt_ack : ∀ m n, m + n < ack m n := by sorry
...
@[target]
theorem lt_ack_right (m n : ℕ) : n < ack m n := by sorry
...
@[target]
theorem ack_injective_left (n : ℕ) : Function.Injective fun m => ack m n := by sorry
...
@[target, simp]
theorem ack_inj_left {m₁ m₂ n : ℕ} : ack m₁ n = ack m₂ n ↔ m₁ = m₂ := by sorry

@[target]
theorem max_ack_left (m₁ m₂ n : ℕ) : ack (max m₁ m₂) n = max (ack m₁ n) (ack m₂ n) := by sorry

@[target]
theorem ack_le_ack {m₁ m₂ n₁ n₂ : ℕ} (hm : m₁ ≤ m₂) (hn : n₁ ≤ n₂) : ack m₁ n₁ ≤ ack m₂ n₂ := by sorry

@[target]
theorem ack_succ_right_le_ack_succ_left (m n : ℕ) : ack m (n + 1) ≤ ack (m + 1) n := by sorry
