...
@[target]
theorem mem_stepSet (s : σ) (S : Set σ) (a : α) : s ∈ M.stepSet S a ↔ ∃ t ∈ S, s ∈ M.step t a := by sorry
...
@[target, simp]
theorem evalFrom_singleton (S : Set σ) (a : α) : M.evalFrom S [a] = M.stepSet S a := by sorry
...
@[target, simp]
theorem eval_nil : M.eval [] = M.start := by sorry
...
theorem toNFA_evalFrom_match (M : DFA α σ) (start : σ) (s : List α) :
    M.toNFA.evalFrom {start} s = {M.evalFrom start s} := by sorry

@[target, simp]
theorem toNFA_correct (M : DFA α σ) : M.toNFA.accepts = M.accepts := by sorry
