...
@[target]
theorem Encoding.encode_injective {α : Type u} (e : Encoding α) : Function.Injective e.encode := by sorry
...
@[target, simp]
theorem sectionΓ'Bool_inclusionBoolΓ' {b} : sectionΓ'Bool (inclusionBoolΓ' b) = b := by sorry
...
theorem Encoding.card_le_card_list {α : Type u} (e : Encoding.{u, v} α) :
    Cardinal.lift.{v} #α ≤ Cardinal.lift.{u} #(List e.Γ) := by sorry
...
@[target]
theorem FinEncoding.card_le_aleph0 {α : Type u} (e : FinEncoding α) : #α ≤ ℵ₀ := by sorry
