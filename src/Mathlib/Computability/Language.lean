...
@[target]
theorem zero_def : (0 : Language α) = (∅ : Set _) := by sorry

@[target]
theorem one_def : (1 : Language α) = ({[]} : Set (List α)) := by sorry

@[target]
theorem add_def (l m : Language α) : l + m = (l ∪ m : Set (List α)) := by sorry

@[target]
theorem mul_def (l m : Language α) : l * m = image2 (· ++ ·) l m := by sorry
...
@[target, ext]
theorem ext {l m : Language α} (h : ∀ (x : List α), x ∈ l ↔ x ∈ m) : l = m := by sorry

@[target, simp]
theorem not_mem_zero (x : List α) : x ∉ (0 : Language α) := by sorry

@[target, simp]
theorem mem_one (x : List α) : x ∈ (1 : Language α) ↔ x = [] := by sorry

@[target]
theorem nil_mem_one : [] ∈ (1 : Language α) := by sorry

theorem mem_add (l m : Language α) (x : List α) : x ∈ l + m ↔ x ∈ l ∨ x ∈ m :=
  Iff.rfl

@[target]
theorem mem_mul : x ∈ l * m ↔ ∃ a ∈ l, ∃ b ∈ m, a ++ b = x := by sorry

@[target]
theorem append_mem_mul : a ∈ l → b ∈ m → a ++ b ∈ l * m := by sorry

@[target]
theorem mem_kstar : x ∈ l∗ ↔ ∃ L : List (List α), x = L.flatten ∧ ∀ y ∈ L, y ∈ l := by sorry

theorem join_mem_kstar {L : List (List α)} (h : ∀ y ∈ L, y ∈ l) : L.flatten ∈ l∗ :=
  ⟨L, rfl, h⟩

@[target]
theorem nil_mem_kstar (l : Language α) : [] ∈ l∗ := by sorry
...
@[target, simp]
theorem map_id (l : Language α) : map id l = l := by sorry

@[target, simp]
theorem map_map (g : β → γ) (f : α → β) (l : Language α) : map g (map f l) = map (g ∘ f) l := by sorry
...
theorem kstar_def_nonempty (l : Language α) :
    l∗ = { x | ∃ S : List (List α), x = S.flatten ∧ ∀ y ∈ S, y ∈ l ∧ y ≠ [] } := by sorry

@[target]
theorem le_iff (l m : Language α) : l ≤ m ↔ l + m = m := by sorry

@[target]
theorem le_mul_congr {l₁ l₂ m₁ m₂ : Language α} : l₁ ≤ m₁ → l₂ ≤ m₂ → l₁ * l₂ ≤ m₁ * m₂ := by sorry
...
@[target]
theorem kstar_eq_iSup_pow (l : Language α) : l∗ = ⨆ i : ℕ, l ^ i := by sorry

@[target, simp]
theorem map_kstar (f : α → β) (l : Language α) : map f l∗ = (map f l)∗ := by sorry
...
@[target, simp]
theorem one_add_kstar_mul_self_eq_kstar (l : Language α) : 1 + l∗ * l = l∗ := by sorry
...
@[target]
lemma reverse_mem_reverse : a.reverse ∈ l.reverse ↔ a ∈ l := by sorry

@[target]
lemma reverse_eq_image (l : Language α) : l.reverse = List.reverse '' l := by sorry
...
@[target]
lemma reverse_injective : Function.Injective (reverse : Language α → _) := by sorry

@[target]
lemma reverse_surjective : Function.Surjective (reverse : Language α → _) := by sorry
...
@[target, simp]
lemma reverse_iInf {ι : Sort*} (l : ι → Language α) : (⨅ i, l i).reverse = ⨅ i, (l i).reverse := by sorry
...
@[target, simp]
lemma reverse_pow (l : Language α) (n : ℕ) : (l ^ n).reverse = l.reverse ^ n := by sorry

@[target, simp]
lemma reverse_kstar (l : Language α) : l∗.reverse = l.reverse∗ := by sorry
